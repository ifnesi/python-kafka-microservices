...
#### **IMPORTANT 1**
Have you noticed the microservice **Deliver Pizza** is statefull as it has two steps?
...
#### **IMPORTANT 2**
The microservice **Process Status** is also stateful as it receives several notifications for the same event key, however that microservice doesn't take into account the statefulness of the system, instead it just processes and pushes through the status changes as they arrive in a stateless fashion. That can be a problem if a given order is not fully processed, for example, what if the baker decided to stop working? The status of the order would get stuck forever as "Your pizza is in the oven". That problem could be solved using **ksqlDB** by using a [session window](https://developer.confluent.io/tutorials/create-session-windows/ksql.html), for example, it could be estimated the orders shouldn't take more than 'X minutes' between being ordered and baked and 'Y minutes' between being baked and not completed yet, if that gets violated it could trigger a notification to state something got stuck (at least the pizza shop manager would get notified before the customer call to complain about the delay).
