...
EXTENSION_AVRO = ".avro"
FOLDER_CONFIG_KAFKA = "config"
FOLDER_CONFIG_SYS = "config"
    ...
    if len(sys.argv) <= 1:
        logging.error(
            (
                ...
                " - SYS_CONFIG_FILE: file under the folder 'config/' (default file is '{DEFAULT_SYS_CONFIG}')\n"
            )
        )
        sys.exit(0)
